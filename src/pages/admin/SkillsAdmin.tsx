  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-secondary">Manage skill groups shown on the public portfolio.</p>
      </div>
      <form onSubmit={save} className="space-y-4 rounded-2xl border border-border bg-surface p-4">
        <input className="w-full rounded-lg border border-border bg-background px-3 py-2" placeholder="Group title" value={form.title} onChange={(e) => setForm((current) => ({ ...current, title: e.target.value }))} />
        <textarea className="w-full rounded-lg border border-border bg-background px-3 py-2" placeholder="Skills JSON preview" value={JSON.stringify(form.skills, null, 2)} onChange={(e) => { try { setForm((current) => ({ ...current, skills: JSON.parse(e.target.value) })); } catch { /* ignore */ } }} />
        <button className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">{editingId ? 'Update group' : 'Add group'}</button>
      </form>
